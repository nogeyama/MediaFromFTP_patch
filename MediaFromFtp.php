...
class MediaFromFtp {
		...
		foreach ($files as $file) {
			...
			if (is_file($fullpath)) {
				if (!preg_match("/".$excludefile."/", $fullpath)) {
					$exts = explode('.', $file);
					$ext = end($exts);
					if (preg_match("/".$searchtext."/", $fullpath)) {
	...
	function get_date_check($file, $dateset){
		...
		echo "get_date_check():file="; print_r($file); echo "\n";
		echo "get_date_check():FILE EXISTS?["; print_r( file_exists($file)); echo "]\n";
		// echo "get_date_check():--> PROBLEM!! Not EXISTS!!\n";
		// ここでファイルが存在しないことが問題！それで時刻が０になる！！★
		
		echo "get_date_check():dateset="; print_r($dateset); echo "\n";
		echo "get_date_check():filemtime()=["; filemtime($file); echo "]\n";
		
		
		$date = get_date_from_gmt(date("Y-m-d H:i:s", filemtime($file)));

		echo "get_date_check():date"; print_r($date); echo "\n";
		...
		if ( is_dir($file) ) { // dirctory
			...
			foreach ( $attachments as $attachment ){
				...
				if ( $attach_url === $new_url || $attach_url === $new_url_md5 ) {
                                        echo "Match:"; print_r( $new_url); echo "]]\n";
					$new_file = FALSE;
				} else {
					if ($thumb_deep_search) {
                                                        echo "[1-3 No Run Here]";
                ...
                //akm                echo "end(input_url) new_url=";
                //akm                print_r( $new_url);
                //akm                echo "\n";
	...
	function regist($ext, $new_url_attach, $new_url_datetime, $dateset, $yearmonth_folders, $character_code, $cron_user){
                ...
                echo "rg():regist start[";   // AKM:
                if (! $new_url_datetime) {
                    echo "ERROR:: if (FALSE) ==>>  File not found! BUG!!]\n";
                }
                else {
                    echo "date OK]\n";
                }
		...
		echo "rg():filename="; print_r($filename); echo "\n";
		...
		//AKM:
		echo "rg():postdategmt="; print_r($postdategmt); echo "\n";
		...
		if ( strpos($currentfile, ' ' ) ) {
			...
			// COPY FILE  // AKM:
			echo "rg():COPY org1:"; print_r($copy_file_org1); echo"\n";
			echo "rg():COPY new1:"; print_r($copy_file_new1); echo"\n";
		...
		if ( function_exists('mb_check_encoding') ) {
			if ( !mb_check_encoding($new_url_attach, 'ASCII') ) {
				...
				// COPY FILE  // AKM:
				echo "rg():COPY org2:"; print_r($copy_file_org2); echo"\n";
				echo "rg():COPY new2:"; print_r($copy_file_new2); echo"\n";
		...
		if ( $yearmonth_folders == 1 ) {
			...
			if ( MEDIAFROMFTP_PLUGIN_UPLOAD_DIR.'/'.$filename <> MEDIAFROMFTP_PLUGIN_UPLOAD_DIR.$subdir.'/'.$filename_base ) {
				...
				// COPY FILE  // AKM:
				echo "rg():COPY org3:"; print_r($copy_file_org3); echo"\n";
				echo "rg():COPY new3:"; print_r($copy_file_new3); echo"\n";
		...
		$attach_id = wp_insert_attachment( $newfile_post, $filename );           // ここでDBに挿入してattach_idを得る！
	 ...
	 * @since	7.4
	 */
	function output_metadata($ext, $attach_id, $metadata, $character_code){
	 ...
	 * @since	9.30
	 */
	function output_html_and_log($ext, $attach_id, $new_attach_title, $new_url_attach, $imagethumburls, $mimetype, $length, $stamptime, $file_size, $exif_text_tag, $image_attr_thumbnail, $mediafromftp_settings){
