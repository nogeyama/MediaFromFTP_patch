...
class MediaFromFtpAjax {
		...
		if (!empty($new_url_attach)) {
			...
			if ( $attach_id == -1 || $attach_id == -2 ) { // error
				...
				if ( $attach_id == -1 ) {
					...
					echo '<div class="error"><ul><li><div>'.__('Title').': '.$error_title.'</div>'.'<div>URL: '.$error_url.'</div><div>'.__('This file could not be registered in the database.', 'media-from-ftp').'</div></li></ul></div>';
				}
			} else {
				// Outputdata
				...
				$output_html = $mediafromftp->output_html_and_log($ext, $attach_id, $new_attach_title, $new_url_attach, $imagethumburls, $mimetype, $length, $stamptime, $file_size, $exif_text_tag, $image_attr_thumbnail, $mediafromftp_settings);
	...
	function mediafromftp_message_callback(){
		...
		if ( $error_count > 0 ) {
			...
			$output_html .= '<div class="error"><ul><li><div>'.$error_message.'</div>'.$error_update.'</li></ul></div>';
		}
		$success_message = sprintf(__('Succeeded to the registration of %1$d files.', 'media-from-ftp'), $success_count);
		$output_html .= '<div class="updated"><ul><li><div>'.$success_message.'</li></ul></div>';
